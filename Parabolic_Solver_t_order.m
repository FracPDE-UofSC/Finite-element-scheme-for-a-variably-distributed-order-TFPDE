function [dt,error_max]  = Parabolic_Solver_t_order(num_of_time,u)
...
% A_fixed = kron(M,kron(M,A))+kron(M,kron(A,M))+kron(A,kron(M,M));
% M_fixed = kron(M,kron(M,M));
% save A_fixed_48_kron A_fixed
% save M_fixed_48_kron M_fixed
% initial solution
A_fixed = A;
M_fixed = M;
...
res = u-uh;
error_max = (h*sum(res.^2))^(1/2);
xx = Pb(1,:);
  Uh_prime = zeros(length(x0),length(t)-1);
for iTime = 1:length(t)-1
   Uh_prime(:,iTime) =(Uh(:,iTime+1) - Uh(:,iTime) )/dt;
end

xx = Pb(1,:);
x_location = find(xx == 1/4);
%% plot x = selet fixed location
x_selected = x_location;
u_x = Uh(x_selected,:);
%
Distri_u = figure
plot(t(1:end),(u_x),'-b','LineWidth',1)
xlabel('${t}$','FontSize', 13,'Interpreter','latex');
ylabel('${u(1/4,t)}$','FontSize', 13,'Interpreter','latex','Rotation',90);
% axis([0 1 0 1]); 
saveas(Distri_u,'Distri_u.eps')
% saveas(h,'jj.jpg');
% title('\bf{u(0.5,t)}','Interpreter','latex')
Distri_v = figure
plot(t(2:end),diff(u_x)/dt,'-r','LineWidth',1)
xlabel('${t}$','FontSize', 13,'Interpreter','latex');
ylabel('${u_{t}(1/4,t)}$','FontSize', 13,'Interpreter','latex','Rotation',90);
% axis([0 1 -1 0]); 
% saveas(Distri_v,'Distri_v.eps')
disp('complete')%% plot
